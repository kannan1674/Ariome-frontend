...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${backendBase}/api/auth/verify-reset-password-otp`, {
      method: 'POST',
    ...
    if (!upstream.ok) {
      ...
      return NextResponse.json({ error: err.message || 'OTP verification failed' }, { status: upstream.status });
    }

    const ok = raw as { message?: string };
    return NextResponse.json(
      {
        HttpResponse: { Message: ok.message || 'OTP verified successfully', StatusCode: 200 },
        Content: email,
      },
      { status: 200 },
    );
