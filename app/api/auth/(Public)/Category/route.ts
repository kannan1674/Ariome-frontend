...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/registration/event-categories?${queryParams.toString()}`;

    const response = await secureServerApiCall(fullUrl, { method: 'GET' }, token ?? undefined);

    if (!response.ok) {
      return createSecureErrorResponse(response.error || 'Failed to fetch event categories', response.status);
    }
