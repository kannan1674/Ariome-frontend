...
export function MegaMenuMobile({ onLinkClick }: { onLinkClick?: () => void }) {
  ...
  const buildMenuItemRoot = (item: MenuItem, index: number): JSX.Element => {
    if (item.children) {
      ...
      return (
        ...
        >
          <Link href={item.path || '#'} className="" onClick={onLinkClick}>
            {item.icon && <item.icon data-slot="accordion-menu-icon" />}
            <div className="flex items-center justify-between grow gap-2">
              <span data-slot="accordion-menu-title">{item.title}</span>
              {item.badge && (
    ...
    if (item.children) {
      ...
      return (
        <AccordionMenuItem
          ...
          className="text-[13px]"
        >
          <Link href={item.path || '#'} onClick={onLinkClick}>
            {item.icon && <item.icon data-slot="accordion-menu-icon" />}
            <div className="flex items-center justify-between grow gap-2">
  ...
  return (
    <div className="flex flex-col grow shrink-0">
      {/* Logo Section */}
      {/* <div className="flex items-center justify-left py-6 px-5 ">
        <Link href="/" className="flex items-center justify-center">
          <img
            src={toAbsoluteUrl('/media/app/cookoo-1024.png')}
            className="dark:hidden"
            style={{ width: '100px' }}
            alt="COOKOO Cycling Club"
          />
          <img
            src={toAbsoluteUrl('/media/app/cookoo-1024.png')}
      ...
      {/* Menu Section */}
      <div className="flex grow shrink-0 py-4  px-3">
        <AccordionMenu
